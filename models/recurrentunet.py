...
from .utils import group_tensor_view, separate_tensor_view
import numpy as np
import torch
...
class RecurrentVarNet(nn.Module):
    ...
    def __init__(
        ...
        in_channels: int = COMPLEX_SIZE * 9,
        num_steps: int = 15,
    ...
    def compute_sense_init(self, kspace: torch.Tensor, sensitivity_map: torch.Tensor) -> torch.Tensor:
        ...
        ----------
        kspace: torch.Tensor
            k-space of shape (N, coil, height, width, nt, complex=2).
        sensitivity_map: torch.Tensor
            Sensitivity map of shape (N, coil, height, width, nt, complex=2).
    ...
    def forward(
        ...
        **kwargs,
        ...
        ----------
        masked_kspace: torch.Tensor
            Masked k-space of shape (N, coil, height, width, nt, complex=2).
        sampling_mask: torch.Tensor
            Sampling mask of shape (N, 1, height, width, 1, 1).
        sensitivity_map: torch.Tensor
            Coil sensitivities of shape (N, coil, height, width, nt, complex=2).
        ...
        #         initializer_input_image = self.backward_operator(masked_kspace,
        #                                                          dim=self._spatial_dims)

            # previous_state = self.initializer(
            #     group_tensor_view(
            #         self.forward_operator(initializer_input_image,  # (nb, nc, kx, ky, nt, 2)
            #                               dim=self._spatial_dims)
            #         .sum(self._coil_dim)                            # (nb, kx, ky, nt, 2)
            #     ).permute(0, 3, 1, 2)                               # (nb, kx, ky, nt * 2) ->   (nb, nt * 2, kx, ky)
            # )

        kspace_prediction = masked_kspace.clone()                   # y0 = \tilde{y}                                # (nb, nc, kx, ky, nt, 2)
        ...
        for step in range(self.num_steps):
            ...
            kspace_prediction, previous_state = block(
                kspace_prediction,                              # y_{t-1}
                masked_kspace,                                  # y_0
                sampling_mask,                                  # U
                sensitivity_map,                                # S
                previous_state,                                 # h_{t-1}
                self._coil_dim,
                self._spatial_dims,
            )
...
class RecurrentVarNetBlock(nn.Module):
    ...
    def __init__(
        ...
        self.regularizer = UnetModel2d(in_channels, in_channels,
                                       num_filters=hidden_channels,
                                       num_pool_layers=num_layers,
    ...
    def forward(
        ...
        ----------
        current_kspace: torch.Tensor
            Current k-space prediction of shape (N, coil, height, width, nt, complex=2).
        masked_kspace: torch.Tensor
            Masked k-space of shape (N, coil, height, width, nt, complex=2).
        sampling_mask: torch.Tensor
            Sampling mask of shape (N, 1, height, width, 1, 1).
        ...
        new_kspace: torch.Tensor
            New k-space prediction of shape (N, coil, height, width, nt, complex=2).
        ...
        recurrent_term = reduce_operator(
            self.backward_operator(current_kspace,
                                   dim=spatial_dims),
            sensitivity_map,
            dim=coil_dim,
        )                                                           # (nb, kx, ky, nt, 2)
        recurrent_term = group_tensor_view(recurrent_term)          # (nb, kx, ky, nt * 2)
        recurrent_term = recurrent_term.permute(0, 3, 1, 2)         # (nb, nt * 2, kx, ky)

        recurrent_term = self.regularizer(recurrent_term)           # :math:`w_t`, :math:`h_{t+1}`
        recurrent_term = recurrent_term.permute(0, 2, 3, 1)         # (nb, kx, ky, nt * 2)
        recurrent_term = separate_tensor_view(recurrent_term)       # (nb, kx, ky, nt, 2)

        recurrent_term = self.forward_operator(
            expand_operator(recurrent_term,                         # (nb, kx, ky, nt, 2)
                            sensitivity_map,                        # (nb, nc, kx, ky, nt, 2)
                            dim=coil_dim),                          # (nb, nc, kx, ky, nt, 2)
            dim=spatial_dims,                                       # (nb, nc, kx, ky, nt, 2)
        )
